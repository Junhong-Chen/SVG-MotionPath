...
const SVG_EL = '#path-svg';
const SVG_STYLE_EL = '#path-svg-style';
const SVG_PATH = '.path-svg_path';
const SVG_TRACE = '.path-svg_flow';
const SVG_NODE = '.path-svg_node';
const SVG_MOBILE = '#path-svg_mobile';

const EASE_LINEAR = 'none';
...
_postAddChecks = function _postAddChecks(timeline, child) {
  ...
  child._start = _roundPrecise$1((_isNumber$1(position) ? position : position || timeline !== _globalTimeline ? _parsePosition(timeline, position, child) : timeline._time) + child._delay);
...
distribute = function distribute(v) {
  ...
  if (_windowExists$1() || config.headless) {
    ...
    config.register && config.register(gsap$1, Plugin, PropTween$1);
  } else {
      ...
      if (specialProp) {
        ...
        if (isTransformRelated) {
          ...
          if (!transformPropTween) {
            ...
            transformPropTween = this._pt = new PropTween$1(this._pt, style, _transformProp$1, 0, 1, cache.renderTransform, cache, 0, -1); //the first time through, create the rendering PropTween so that it runs LAST (in the linked list, we keep adding to the beginning)
...
function getPositionOnPath$1(rawPath, progress, includeAngle, point) {
...
var MotionPathPlugin = {
  ...
  render: function render(ratio, data) {
    ...
    if (data.tween._time || !_reverting()) {
      ...
      while (i--) {
        getPositionOnPath$1(rawPaths[i], ratio, !i && data.rotate, rawPaths[i]);
      }
  ...
  getPositionOnPath: getPositionOnPath$1,
...
class SVGMotionPath {
  ...
  constructor(container, data, { motion = {}, svg = {}, style = {} }) {
    ...
    if (flowMotion.duration) {
      ...
      this.#flow = gsapWithCSS.to(SVG_TRACE, {
        strokeDashoffset,
        repeat: -1,
        ease: EASE_LINEAR,
...
export { SVGMotionPath as default };
